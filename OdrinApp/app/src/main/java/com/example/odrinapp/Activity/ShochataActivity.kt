...
class ShochataActivity : AppCompatActivity(), LoaderManager.LoaderCallbacks<Cursor> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        drinnkName.text = "Hochata"
        imageView.setImageResource(R.drawable.irisretto)

        addtoCart.setOnClickListener {
            val intent = Intent(this@ShochataActivity, SummaryActivity::class.java)
        ...
        minusquantity.setOnClickListener {
            ...
            if (quantity == 0) {
                Toast.makeText(this@ShochataActivity, "Can't decrease quantity below 0", Toast.LENGTH_SHORT).show()
            } else {
                quantity--
                displayQuantity()
                val coffeePrice = basePrice * quantity
    ...
    private fun saveCart() {
        ...
        if (mCurrentCartUri == null) {
            ...
            if (newUri == null) {
                Toast.makeText(this@ShochataActivity, "Failed to add to Cart", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this@ShochataActivity, "Success adding to Cart", Toast.LENGTH_SHORT).show()
            }
        }
