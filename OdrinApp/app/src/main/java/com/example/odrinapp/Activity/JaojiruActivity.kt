...
class JaojiruActivity : AppCompatActivity(), LoaderManager.LoaderCallbacks<Cursor> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        drinnkName.text = "Aojiru Tea"
        imageView.setImageResource(R.drawable.jaojiru)

        addtoCart.setOnClickListener {
            val intent = Intent(this@JaojiruActivity, SummaryActivity::class.java)
        ...
        minusquantity.setOnClickListener {
            ...
            if (quantity == 0) {
                Toast.makeText(this@JaojiruActivity, "Can't decrease quantity below 0", Toast.LENGTH_SHORT).show()
            } else {
                quantity--
                displayQuantity()
                val coffeePrice = basePrice * quantity
    ...
    private fun saveCart() {
        ...
        if (mCurrentCartUri == null) {
            ...
            if (newUri == null) {
                Toast.makeText(this@JaojiruActivity, "Failed to add to Cart", Toast.LENGTH_SHORT).show()
            } else {
                Toast.makeText(this@JaojiruActivity, "Success adding to Cart", Toast.LENGTH_SHORT).show()
            }
        }
